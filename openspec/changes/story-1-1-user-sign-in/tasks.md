...
- [ ] 6.2 RED: wrong password / inactive user / inactive tenant return identical `AUTH_INVALID_CREDENTIALS` -> GREEN: `src/modules/auth/server/auth.ts` (D11)
- [ ] 6.3 RED: nickname resolves to email inside scoped tx -> GREEN: `src/modules/auth/server/sign-in.action.ts` (D7)
- [ ] 6.4 RED: same email succeeds across two tenants -> GREEN: confirm composite uniqueness (D4)
